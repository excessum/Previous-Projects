 ...
 ***********************************************************************************************/
		...
		MPI_Sendrecv (&sendbuf[0],myrowsize,MPI_INT, left,tag,
			&recvbuf[0], myrowsize, MPI_INT, right, tag,
			new_comm, &status);
			
		for(i = 1; i< myrowsize; i++){
			R[i][mycolsize+1] = recvbuf[i-1];
		}
		for(i = 1; i< myrowsize; i++){
			sendbuf[i-1] = R[i][mycolsize];
		}
		MPI_Sendrecv (&sendbuf[0],myrowsize,MPI_INT, right,tag,
			&recvbuf[0], myrowsize, MPI_INT, left, tag,
			new_comm, &status);
		for(i = 1; i< myrowsize; i++){
			R[i][0] = recvbuf[i-1];
		}
		...
		for(i = 1; i< myrowsize; i++){
			G[i][mycolsize+1] = recvbuf[i-1];
		}
		for(i = 1; i< myrowsize; i++){
			sendbuf[i-1] = G[i][mycolsize];
		}
		MPI_Sendrecv (&sendbuf[0],myrowsize,MPI_INT, right,tag,
			&recvbuf[0], myrowsize, MPI_INT, left, tag,
			new_comm, &status);
		...
		MPI_Sendrecv (&sendbuf[0],myrowsize,MPI_INT, left,117,
			&recvbuf[0], myrowsize, MPI_INT, right, 117,
			new_comm, &status);
			
		for(i = 1; i< myrowsize; i++){
			B[i][mycolsize+1] = recvbuf[i-1];
		}
		for(i = 1; i< myrowsize; i++){
			sendbuf[i-1] = B[i][mycolsize];
		}
		MPI_Sendrecv (&sendbuf[0],myrowsize,MPI_INT, right,118,
			&recvbuf[0], myrowsize, MPI_INT, left, 118,
			new_comm, &status);
		for(i = 1; i< myrowsize; i++){
			B[i][0] = recvbuf[i-1];
		}
